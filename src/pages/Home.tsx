...
const DEFAULT_NEWS_FALLBACK = [
  { id: 1, title: "Sinergi IKA PMII KBB dengan Pemkab Bandung Barat dalam Program Penataan Desa", content: "PC IKA PMII Kabupaten Bandung Barat menjalin kesepakatan strategis dengan Pemerintah Kabupaten Bandung Barat untuk mendorong digitalisasi administrasi dan pemberdayaan ekonomi di tingkat desa se-KBB.", image: pmiiMeetingImg, date: new Date().toISOString(), category: "Berita", author: "Humas IKA PMII" },
  { id: 2, title: "Silaturahim Akbar & Consolidasi Alumni Menyongsong Rapat Kerja Cabang", content: "PC IKA PMII Kabupaten Bandung Barat menyelenggarakan kegiatan silaturahim akbar guna mempererat hubungan kekeluargaan antar-alumni sekaligus merapatkan barisan menyongsong Rapat Kerja Cabang.", image: pmiiMeetingImg, date: new Date().toISOString(), category: "Berita", author: "Redaksi" },
    ...
    fetch("/api/content/news")
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const sanitized = data.slice(0, 5).map((item: any) => {
